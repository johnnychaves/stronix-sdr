...
| **Lock por phone no buffer** | 🟡 Resolve race conhecida | Hoje, se lead manda 2ª msg durante o sleep de 1-3min de digitação, gera 2 chamadas Claude paralelas. Known issue desde 2026-05-01. ~30 linhas de código, low-risk. Não bloqueia rollout v2 mas vale fechar antes de escalar pra >100 leads/dia. |
| **Verificar tier Anthropic** | 🟡 Operacional | Confirmar se conta está em Tier 1 (50 req/min Sonnet) ou superior. Pico extremo (>50 msgs/min) gera 429. Tier 2 ($40 deposit) dobra pra 100/min. Levantamento técnico de 2026-05-02 mostra folga de 10x no volume atual. |
| Buffer persistente em DB | 🟢 Baixa | Hoje buffer é in-memory. Restart perde msgs pendentes (raro em Railway). Só relevante se >100 leads/dia. |
| Queue dedicada (Redis + BullMQ) + Node clusterizado | 🟢 Baixa | Só se passar de 500 leads/dia. STRONIX hoje em ~30/dia tem folga grande. |
