...
// Baixa um áudio recebido (Meta media_id) → Buffer + mimeType.
// Usado pelo transcribe quando provider=meta.
async function downloadMediaToBuffer(mediaId) {
  ...
  return { buffer: Buffer.from(audio.data), mimeType: metaRes.data.mime_type || 'application/octet-stream' };
}

module.exports = { sendMessage, sendAudio, uploadMedia, transcodeAudio, downloadMediaToBuffer, notifyOwner, notifyStudent, notifyAssignedConsultor };
